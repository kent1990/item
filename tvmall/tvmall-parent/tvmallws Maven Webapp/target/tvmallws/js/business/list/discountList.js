...
function funKeyPressOk(){
    ...
    window.location.href = "toGoodsDetail.do?goodsId="+dataList[startId + focusId].goodsId
    						+"&type="+dataList[startId + focusId].type +"&pageTitle= 打折促销 | 商品详情&r="+Math.random();    
}
	...
	setCont : function($this,startId, len){
		...
		for(var dataIndex = startId, index = 0; dataIndex < startId + len; dataIndex++){
			 ...
			 var pic_html = '<div class="pic">'+
			    '<img src="view.do?path=' + dataList[dataIndex].picPath + '" alt='+dataList[dataIndex].goodsName+'></div>';
			 var desc_html = '<div class="desc"><div class="price">' + dataList[dataIndex].goodsPromotionPrice + '</div><div class="name">' + dataList[dataIndex].goodsName + '</div></div>';          
			 var adom = "#div_0_" +index;						
