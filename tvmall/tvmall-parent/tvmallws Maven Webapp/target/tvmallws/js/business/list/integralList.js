...
function funKeyPressOk(){
    ...
    window.location.href = "toGoodsDetail.do?goodsId="+dataList[startId + focusId].goodsId
						+"&type="+dataList[startId + focusId].type +"&pageTitle= 积分商城 | 商品详情&r="+Math.random();    
}
    ...
    */
	setCont : function($this,startId, len){
		...
		for(var dataIndex = startId, index = 0; dataIndex < startId + len; dataIndex++){
			 ...
			 var desc_html = '<div class="desc"><div class="price">' + dataList[dataIndex].goodsPromotionPrice + '</div><div class="name">' 
			 + dataList[dataIndex].goodsName + '</div><div class="price">'+dataList[dataIndex].goodsExchIntegral+'</div></div>';           
			    var adom = "#div_0_" +index;		
