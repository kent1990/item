...
layui.define(['jquery', 'jqelem'], function(exports) {
     ...
     */
    tabMenu.prototype.tabAdd = function(data, fresh, freshUrl) {
        ...
        if (tab_index === -1) {
            ...
            if (fresh && freshUrl != null) {
				if (freshUrl == undefined || freshUrl == null || freshUrl == '') {
					_this.fresh(tab_index);
				} else {
					_this.fresh2(tab_index, freshUrl);
				}
			}
        }
     ...
     * @param freshUrl
     */
    tabMenu.prototype.fresh2 = function(index, freshUrl) {
		element.tabChange(objTab.tabFilter, index);
		var othis = objTab.titleBox.find('>li[lay-id="' + index + '"]'),
			index = othis.parent().children('li').index(othis),
			parents = othis.parents('.layui-tab').eq(0),
			item = parents.children('.layui-tab-content').children('.layui-tab-item'),
			src = item.eq(index).find('iframe').attr("src");
		item.eq(index).find('iframe').attr("src", freshUrl);
	}
