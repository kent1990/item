...
function funKeyPressOk(){
    ...
    window.location.href = "toHelpDetail.do?helpId="+dataList[startId + focusId].helpId+"&r="+Math.random();    
}
	...
	setCont : function($this,startId, len){
		...
		for(var dataIndex = startId, index = 0; dataIndex < startId + len; dataIndex++){
			 ...
			 var pic_html = '<div class="pic">'+
			    '<img src="view.do?path=' + dataList[dataIndex].picPath + '" alt='+dataList[dataIndex].helpName+'></div>';
			    var desc_html = '<div class="desc"><div class="name">' + dataList[dataIndex].helpName + '</div></div>';          
			    var adom = "#div_0_" +index;		
