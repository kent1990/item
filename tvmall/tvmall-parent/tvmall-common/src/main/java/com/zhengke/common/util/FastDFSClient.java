...
public class FastDFSClient {
	...
	static {
		try {
			String configFile = CommonUtil.getClassesPath()+CONF_FILENAME;
			logger.info("=== CONF_FILENAME:" +configFile);
	...
	public static String uploadFile(byte[] fileArray, String fileName) {
		FileInputStream fis = null;
		try {
			NameValuePair[] meta_list = null; // new NameValuePair[0];
			
			String fileid = storageClient1.upload_file1(fileArray, getFileExt(fileName), meta_list);
			return fileid;
		} catch (Exception ex) {
			logger.error(ex);
			return null;
